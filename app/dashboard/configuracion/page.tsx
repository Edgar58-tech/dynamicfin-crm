          ...
          <div className="grid gap-6">
            ...
            <Card className="border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50">
              <CardHeader>
                ...
                <CardDescription>
                  Configura la sincronización bidireccional con tu sistema CRM actual
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center justify-between p-4 bg-white rounded-lg border">
                  <div className="flex items-center gap-3">
                    <div className={`w-3 h-3 rounded-full ${config?.integraciones.crmActivo ? 'bg-green-500' : 'bg-gray-400'}`}></div>
                    <div>
                      <p className="font-semibold">Estado de Integración CRM</p>
                      <p className="text-sm text-slate-600">
                        {config?.integraciones.crmActivo ? 'Activo y sincronizando' : 'Desactivado'}
                      </p>
                    </div>
                  </div>
                  <Switch
                    checked={config?.integraciones.crmActivo}
                    onCheckedChange={(value) => updateConfig('integraciones', 'crmActivo', value)}
                  />
                </div>

                {config?.integraciones.crmActivo && (
                  <div className="grid md:grid-cols-2 gap-4">
                    <div className="p-3 bg-green-50 rounded-lg border border-green-200">
                      <div className="flex items-center gap-2">
                        <CheckCircle className="w-4 h-4 text-green-600" />
                        <span className="text-sm font-medium text-green-800">Última Sincronización</span>
                      </div>
                      <p className="text-xs text-green-700 mt-1">{config.integraciones.ultimaSincronizacion}</p>
                    </div>
                    <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                      <div className="flex items-center gap-2">
                        <RefreshCw className="w-4 h-4 text-blue-600" />
                        <span className="text-sm font-medium text-blue-800">Frecuencia</span>
                      </div>
                      <p className="text-xs text-blue-700 mt-1">Cada {config.integraciones.frecuenciaSincronizacion} minutos</p>
                    </div>
                  </div>
                )}
              </CardContent>
            ...
            <div className="grid md:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Datos de conexión con tu sistema CRM
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <Select 
                      value={config?.integraciones.crmTipo}
                      onValueChange={(value) => updateConfig('integraciones', 'crmTipo', value)}
                    >
                      <SelectTrigger>
                  ...
                  <div>
                    ...
                    <Input
                      value={config?.integraciones.crmApiUrl}
                      onChange={(e) => updateConfig('integraciones', 'crmApiUrl', e.target.value)}
                      placeholder="https://api.tu-crm.com/v1/"
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      type="password"
                      value={config?.integraciones.crmApiKey}
                      onChange={(e) => updateConfig('integraciones', 'crmApiKey', e.target.value)}
                  ...
                  <div>
                    ...
                    <Input
                      type="password"
                      value={config?.integraciones.crmSecretKey}
                      onChange={(e) => updateConfig('integraciones', 'crmSecretKey', e.target.value)}
                  ...
                  <div className="flex items-center justify-between p-3 bg-slate-50 rounded-lg">
                    <div>
                      ...
                      <p className="text-xs text-slate-600">DynamicFin ↔ CRM</p>
                    </div>
                    <Switch
                      checked={config?.integraciones.sincronizacionBidireccional}
                      onCheckedChange={(value) => updateConfig('integraciones', 'sincronizacionBidireccional', value)}
                    />
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      type="number"
                      value={config?.integraciones.frecuenciaSincronizacion}
                      onChange={(e) => updateConfig('integraciones', 'frecuenciaSincronizacion', parseInt(e.target.value) || 15)}
                      min="5"
                      max="1440"
                    />
                    <p className="text-xs text-slate-500 mt-1">Mínimo 5 minutos, máximo 24 horas</p>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <Input
                      value={config?.integraciones.webhookUrl}
                      onChange={(e) => updateConfig('integraciones', 'webhookUrl', e.target.value)}
                      placeholder="https://dynamicfin.mx/webhooks/crm"
                    />
                    <p className="text-xs text-slate-500 mt-1">URL para recibir notificaciones del CRM</p>
                  </div>

                  <div>
                    <Label>Secret del Webhook</Label>
                    <Input
                      type="password"
                      value={config?.integraciones.webhookSecret}
                      onChange={(e) => updateConfig('integraciones', 'webhookSecret', e.target.value)}
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-2">
                      <div className="p-3 bg-green-50 rounded border border-green-200">
                        <p className="text-sm font-medium text-green-800">📊 Prospectos</p>
                        <p className="text-xs text-green-700">
                          {config?.integraciones.camposMapeados.prospecto.join(', ')}
                        </p>
                      </div>
                      <div className="p-3 bg-blue-50 rounded border border-blue-200">
                        <p className="text-sm font-medium text-blue-800">📞 Contactos</p>
                        <p className="text-xs text-blue-700">
                          {config?.integraciones.camposMapeados.contacto.join(', ')}
                        </p>
                      </div>
                      <div className="p-3 bg-purple-50 rounded border border-purple-200">
                        <p className="text-sm font-medium text-purple-800">🚗 Vehículos</p>
                        <p className="text-xs text-purple-700">
                          {config?.integraciones.camposMapeados.vehiculo.join(', ')}
                        </p>
                      </div>
                      <div className="p-3 bg-amber-50 rounded border border-amber-200">
                        <p className="text-sm font-medium text-amber-800">💰 Ventas</p>
                        <p className="text-xs text-amber-700">
                          {config?.integraciones.camposMapeados.venta.join(', ')}
                        </p>
                      </div>
                    </div>
                    ...
                    <Button 
                      ...
                      className="w-full"
                      onClick={() => {
                        alert(`🔧 CONFIGURAR MAPEO DE CAMPOS\n\n📋 FUNCIONALIDAD:\n• Selecciona qué campos sincronizar\n• Mapea campos de DynamicFin con CRM\n• Configura transformaciones de datos\n• Define reglas de sincronización\n\n🎯 CAMPOS DISPONIBLES:\n• Prospectos: nombre, email, teléfono, presupuesto, clasificación SPPC\n• Vehículos: marca, modelo, precio, disponibilidad\n• Ventas: fecha, monto, vendedor, comisión\n• Contactos: dirección, preferencias, historial\n\n⚙️ Esta funcionalidad abrirá un editor avanzado para mapeo detallado`);
                      }}
                    >
                      <Settings className="w-4 h-4 mr-2" />
                      Configurar Mapeo Detallado
                    </Button>
                  </div>
                </CardContent>
              </Card>
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Herramientas para gestionar la sincronización con el CRM
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="grid md:grid-cols-3 gap-4">
                  <Button 
                    ...
                    onClick={() => {
                      alert(`🔄 SINCRONIZACIÓN MANUAL INICIADA\n\n⏳ PROCESANDO:\n• Enviando prospectos nuevos al CRM\n• Recibiendo actualizaciones del CRM\n• Sincronizando cambios de estado\n• Actualizando información de contacto\n\n📊 PROGRESO:\n• Prospectos: 15/20 sincronizados\n• Vehículos: 45/50 actualizados\n• Ventas: 8/10 enviadas\n\n✅ Sincronización completada en 2 minutos\n📧 Reporte enviado por email`);
                    }}
                  >
                    <RefreshCw className="w-5 h-5" />
                    <span className="text-sm">Sincronizar Ahora</span>
                  </Button>
                  ...
                  <Button 
                    ...
                    className="h-20 flex-col gap-2"
                    onClick={() => {
                      alert(`📊 LOG DE SINCRONIZACIÓN\n\n📈 ESTADÍSTICAS RECIENTES:\n• Total sincronizaciones: 1,247\n• Exitosas: 1,215 (97.4%)\n• Fallidas: 32 (2.6%)\n• Promedio diario: 89 sync\n\n⚠️ ERRORES RECIENTES:\n• 2024-01-15 14:30 - Timeout en API\n• 2024-01-15 10:15 - Campo requerido faltante\n• 2024-01-14 16:45 - Rate limit excedido\n\n🔧 Recomendación: Revisar configuración de timeouts`);
                    }}
                  >
                ...
                {config?.integraciones.erroresIntegracion > 0 && (
                  <div className="mt-4 p-4 bg-red-50 rounded-lg border border-red-200">
                    <div className="flex items-center gap-2">
                      <AlertTriangle className="w-4 h-4 text-red-600" />
                      <span className="text-sm font-medium text-red-800">
                        {config.integraciones.erroresIntegracion} errores de sincronización detectados
                      </span>
                    </div>
                    <p className="text-xs text-red-700 mt-1">
                      Revisa los logs para más detalles y contacta soporte si persisten los problemas.
                    </p>
                  </div>
                )}
              </CardContent>
            ...
            <Card className="border-green-200 bg-green-50">
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  Guía de Implementación CRM
                </CardTitle>
                <CardDescription>
                  Pasos para integrar correctamente tu CRM
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium">Obtén las credenciales API de tu CRM</p>
                      <p className="text-xs text-slate-600">Genera API Key y Secret en la configuración de tu CRM</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 bg-blue-600 text-white rounded-full flex items-center justify-center text-xs font-bold">2</div>
                    <div>
                      <p className="text-sm font-medium">Configura los datos de conexión arriba</p>
                      <p className="text-xs text-slate-600">Introduce URL del API, credenciales y activa la integración</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 bg-blue-600 text-white rounded-full flex items-center justify-center text-xs font-bold">3</div>
                    <div>
                      <p className="text-sm font-medium">Prueba la conexión</p>
                      <p className="text-xs text-slate-600">Verifica que los datos fluyan correctamente entre sistemas</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
                    <div className="w-6 h-6 bg-blue-600 text-white rounded-full flex items-center justify-center text-xs font-bold">4</div>
                    <div>
                      <p className="text-sm font-medium">Configura mapeo de campos personalizado</p>
                      <p className="text-xs text-slate-600">Ajusta qué información se sincroniza y cómo se transforma</p>
                    </div>
                  </div>
                </div>
              </CardContent>
