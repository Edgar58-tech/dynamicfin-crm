...
export async function POST(request: NextRequest) {
  try {
    ...
    // Extract rolePlaySession data
    const rolePlaySession = body.rolePlaySession || null

    // Validar que la sesión existe
    if (!rolePlaySession) {
      return NextResponse.json(
        { error: 'No roleplay session provided' },
        { status: 400 }
      )
    }
      ...
      start(controller) {
        try {
          ...
          const progressData = JSON.stringify({
            status: 'streaming',
            content: buffer,
            sessionId: rolePlaySession?.id || 0,  // ✅ CORREGIDO - Era línea 192
            totalContent: buffer
          })
          ...
          setTimeout(async () => {
            try {
              ...
              await prisma.rolePlaySession.update({
                where: { id: rolePlaySession?.id || 0 },  // ✅ CORREGIDO - Era línea 226
                data: {
              ...
              const finalData = JSON.stringify({
                status: 'completed',
                sessionId: rolePlaySession?.id || 0,  // ✅ CORREGIDO - Era línea 238
              ...
              const errorData = JSON.stringify({
                status: 'error',
                sessionId: rolePlaySession?.id || 0,  // ✅ NULL-SAFE
                error: errorMessage
              })
...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      ...
      status: 'active',
      totalMessages: session.totalMensajes || 0,
      lastActivity: session.fechaUltimaActividad
    })
